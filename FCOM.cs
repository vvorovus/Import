...
namespace Import
{
    public class FCOM
    {
        ...
        public static void Parse(System.Xml.XmlDocument doc)
        {
            ...
            int pointcodeID = App.ExecuteInt32("select id from pointcode where factory = '{0}' and sourcetype = {1}", number, sourcetypeID);
            if (pointcodeID == 0)
            ...
            {
                indication = Double.Parse(day.Attributes["value"].Value, provider);
                ...
                foreach (XmlElement period in day.SelectNodes("./period"))
                {
                    ...
                    double value = Double.Parse(period.Attributes["value"].Value, provider);
                    indication = Math.Round(indication + value, 5); // показания на конец текущего периода
                    // точность счетчика СТЕБ в части показаний 5 знаков после запятой
                    value *= koeftrans;

                    App.InsertPeriod(datasourceID, begin, end, (float)value, (float)indication);

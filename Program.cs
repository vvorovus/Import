...
namespace Import
{
    ...
    class App
    {
        ...
        public static int InsertParent(int CustomerID, string name, int TariffTypeID)
        {
            int pointID = App.ExecuteInt32("select MAX(id) from point");
            pointID++;

            ExecuteNonQuery("insert into point (id,name,number) values ({0},'{1}',{2})", pointID, name, CustomerID);
            ExecuteNonQuery("insert into pointCustomer (point, CustomerID, gtp, profile,TariffTypeID) values ({0},{1},1,1)", pointID, CustomerID,TariffTypeID);
            ExecuteNonQuery("insert into datasource (point, channel, sourcetype ) values ({0},1,7)", pointID);
            ExecuteNonQuery("insert into datasource (point, channel, sourcetype ) values ({0},1,13)", pointID);
        ...
        public static void InsertLKYUR(int SourceID, int CustomerID,int ObjectID,string Channel, string Factory, string DataWrite,string TariffNumber, float MeterCharge)            
        {
            NumberFormatInfo provider = new NumberFormatInfo();
            provider.NumberDecimalSeparator = ".";

            if (CustomerID == 5376)
            App.ExecuteNonQuery("exec [INSERT_LKYUR] {0},{1},{2},'{3}','{4}','{5}','{6}',{7}",
                 SourceID, CustomerID, ObjectID, Channel, Factory, DataWrite, TariffNumber, MeterCharge.ToString(provider));
            else
                App.ExecuteNonQuery("exec [INSERT_LKYUR] {0},{1},{2},'{3}','{4}','{5}','{6}',{7}",
                 SourceID, CustomerID, ObjectID, Channel, Factory, DataWrite, TariffNumber, MeterCharge.ToString(provider));
        }

...
namespace Import
{
    ...
    {       
        ...
        public const string A_plus = "Активная прямая (кВт)";
        public const string A_minus = "Активная обратная (кВт)";
        public const string R_plus = "Реактивная прямая (квар)";
        public const string R_minus = "Реактивная обратная (квар)";
        public const string Metka = "Метка времени";
        public const string PointNumber = "Серийный номер";
        ...
        public static void Parse(string FileName)
        {
            ...
            if (pointID == 0) throw new Exception("Точка \"" + pointNumber.ToString() + "\" не найдена");
            ...
            for (int irow = IndexRow+1; irow < sheet.Rows; irow++)
            {
                try
                {
                    ...
                    a_plus = StringToFloat(C.Str(sheet.getCell(a_plusNumber, irow).Value))*koeftrans;
                    r_plus = StringToFloat(C.Str(sheet.getCell(r_plusNumber, irow).Value)) * koeftrans;
                    a_minus = StringToFloat(C.Str(sheet.getCell(a_minusNumber, irow).Value)) * koeftrans;
                    r_minus = StringToFloat(C.Str(sheet.getCell(r_minusNumber, irow).Value)) * koeftrans;
